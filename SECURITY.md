...
A browser-based prototype for issuing, verifying, and revoking digital passports for artworks: ECDSA signing, perceptual-hash duplicate detection, and human-review routing, all client-side. Originally by @ChristineLumen.
...
The C2PA detection in `index.html` is a **substring search, not a signature verification**: a file containing the bytes `c2pa` or `jumb` anywhere in its first 200 KB is treated as carrying provenance metadata. This affects how a file is routed for review. It is documented in the peer review and is not yet fixed — please do not report it as new, but do tell us if you find a further consequence of it.

The forensic signals in this prototype are explicitly advisory. The page's own caveat explains which of them testing falsified.
